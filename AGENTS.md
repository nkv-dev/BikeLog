...
Every entry file:
```
# <title line>                 <- display only
<optional > description>       <- display only
<sub>Auto-generated by BikeLog …</sub>
<!-- bikelog:<category> -->    <- FIRST marker classifies the file (bike|fuel|maintenance|ride|modification|checklist)
- key: value                  <- props directly under the marker
```

- Properties are `- key: value` lines. Optional props may be omitted. `items` is a
  comma-separated list. Keys match app types; unknown keys ignored on read.
- The `#` heading + `> description` are **display-only** (ignored on parse).

> ⚠️ **Known quirk:** the serializer writes the service marker as
> `<!-- bikelog:service -->` (from `serviceToMarkdown`) while the parser
> `markdownToService` expects `category === "service"` — they match. However the DATA.md
> examples show `bikelog:maintenance`. When adding a new category be careful: the marker
> string and the parser's expected `category` string **must** be exactly equal.

### Strict keys per category

- **Fuel:** `pricePerLitre, totalCost, isFullTank, station?, notes?`
- **Service:** `type (scheduled|repair|general|issue), items[] (comma), cost, workshop?,
  nextServiceKm?, nextServiceDate?`
- **Issue** (type=issue): `title, description, severity, status, estimatedCost?, actualCost?, notes?`
- **Bike:** `photos?: string[]`
- **Ride:** `route, distanceKm, odometer?, avgSpeedKmh?, notes?`
- **Mod:** `title, description?, cost?, installedBy?`
...
- **Photos** upload to `bikelog/bikes/<slug>/media/`; referenced in `bike.md` `photos` list.
...
5. **Theme marker consistency:** serializer marker string and parser expected `category`
   must be identical per category (see §6 quirk).
